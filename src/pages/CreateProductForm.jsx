  ...
  useEffect(() => {
    if (subCategoryId && catByIdData) {
      const selectProductName =
        catByIdData?.subCategories.find((item) => item._id === subCategoryId)
          ?.name || "N/A";
        ...
        <div className="md:col-span-1 lg:col-span-1 bg-transparent border-0 p-6 xs:grid xs:grid-cols-2 gap-6 space-y-4">
          <div className="col-span-1 align-center sm:block flex flex-col justify-center">
            ...
            <p className="text-[13px] text-muted-foreground text-center">
              ...
              below. If this isn't the category you meant to choose, you can go
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div className="w-full max-w-md border-[1.5px] border-gray-200 rounded-lg bg-white p-3">
                          <div className="flex justify-between items-center mb-3">
                            <Label className="font-medium text-gray-500">
                              Old Product{" "}
                              <sup className="hidden sm:block">(in Years)</sup>
                            </Label>
                            <XIcon
                          ...
                          <div className="flex justify-between items-center mt-3 text-sm">
                            <div className="flex items-center gap-2">
                              <Label className="text-gray-600 text-sm hidden sm:block">
                                Min.
                              </Label>
              ...
              {productField === "new_product" &&
                currentCategoryName === "furniture" && (
                  <Select
                    ...
                    onValueChange={(value) =>
                      setValue("conditionOfProduct", value)
                    }
                  >
                    <SelectTrigger className="w-full bg-white">
                      <SelectValue placeholder="Product Condition" />
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="for_parts_or_repair">
                        For Parts Or Repair
                      </SelectItem>
              ...
              {currentCategoryName === "industrial" && (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="industrial_tool">
                      Industrial Tool
                    </SelectItem>
                    <SelectItem value="machinery_tool">
                      Machinery Type
                    </SelectItem>
                  </SelectContent>
                </Select>
          ...
          <div className="rounded-[5px] p-6 bg-gray-200/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              >
                ...
                />
                {image && (
                  <p className="text-xs mt-2 text-green-600">{image?.name}</p>
                )}
              ...
              >
                ...
                <span className="text-sm text-muted-foreground text-center">
                  ...
                  <span className="text-xs">
                    to upload your doc/pdf (max 5 MB)
                  </span>
                </span>
                <input
                  type="file"
                ...
                />
                {fileDoc && (
                  <p className="text-xs mt-2 text-green-600">{fileDoc.name}</p>
                )}
...
const CreateProductForm = () => {
  ...
  const { fn: getCatByIdFn, data: catByIdData } = useFetch(
    categoryService.getCategoriesById,
  );
  const { fn: createProduct, data: createProductRes } = useFetch(
  ...
  const validateForm = (formData, isDraft) => {
    ...
    if (
      !formData.brand &&
      currentCategoryName?.toLowerCase() !== "service" &&
      !isDraft
    ) {
    ...
    if (
      formData.gst_requirement === "yes" &&
      formData.paymentAndDelivery?.gstNumber &&
      !isDraft
    ) {
      if (!gstRegex.test(formData.paymentAndDelivery.gstNumber.trim())) {
        toast.error(
          "Invalid GST Number format. Must be 15 characters (Format: 22AAAAA0000A1Z5)",
        );
        return false;
      }
    }
  ...
  const handleSubmit = async (isDraft) => {
    ...
    toast.success(
      `Form ${isDraft ? "saved as draft" : "submitted"} successfully!`,
    );
  };
  useEffect(()=>{
    if(createProductRes){
