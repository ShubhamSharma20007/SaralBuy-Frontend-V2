  ...
  return (
    <div className={` px-8 sm:px-16 relative bg-no-repeat z-0 bg-cover  py-10  min-h-82 `}>
      ...
      <div className="flex justify-between items-center max-w-7xl mx-auto px-4">
        <p
          className={`font-bold text-3xl  text-${bg || 'orange'}-700 border-l-5 border-${bg || 'orange'}-700 pl-3 tracking-tight capitalize`}
        >
          {item?.categoryName}
        </p>
        {/* {
            ...
            <div className="flex  justify-between mt-5 items-center flex-row-reverse">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center text-sm text-gray-700 gap-2">
                  ...
                  <span className="capitalize">{mergeName(item?.userInfo)}</span>
                </div>
                <div className="flex items-center text-sm text-gray-700 gap-2">
                  {/* map */}
                  <svg
                  ...
                  <span className="capitalize">
                    {item?.userInfo?.currentLocation || item?.userInfo?.address}
                  </span>
                </div>
                <div className="flex items-center text-sm text-gray-700 gap-2">
                  {/* list */}
                  ...
                  {item?.quantity} units{' '}
                  <span className="capitalize">{item?.userInfo?.quantity}</span>
                </div>
              </div>
            </div>
            <div className="flex flex-row items-center justify-between mt-3"></div>
          </div>
        ))}
      </div>
    </div>
