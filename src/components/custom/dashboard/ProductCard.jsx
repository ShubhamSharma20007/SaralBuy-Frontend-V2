...
const ProductCard = ({product:item }) => {
  ...
  return (
      ...
      <div className="flex justify-between items-center  mb-4 ">
        <span className="border-2 border-gray-600 rounded-full text-gray-700 inline-block p-1  text-center px-4 text-sm font-medium capitalize"> {item?.productId?.categoryId?.categoryName || item?.categoryId?.categoryName}</span>
        {
...
</svg> {item?.currentLocation || item?.buyerId?.address || 'N/A'}
            </div>
            <div className="flex items-center text-sm text-gray-700 gap-2">
         <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4 text-gray-500">
  <path fillRule="evenodd" d="M2.625 6.75a1.125 1.125 0 1 1 2.25 0 1.125 1.125 0 0 1-2.25 0Zm4.875 0A.75.75 0 0 1 8.25 6h12a.75.75 0 0 1 0 1.5h-12a.75.75 0 0 1-.75-.75ZM2.625 12a1.125 1.125 0 1 1 2.25 0 1.125 1.125 0 0 1-2.25 0ZM7.5 12a.75.75 0 0 1 .75-.75h12a.75.75 0 0 1 0 1.5h-12A.75.75 0 0 1 7.5 12Zm-4.875 5.25a1.125 1.125 0 1 1 2.25 0 1.125 1.125 0 0 1-2.25 0Zm4.875 0a.75.75 0 0 1 .75-.75h12a.75.75 0 0 1 0 1.5h-12a.75.75 0 0 1-.75-.75Z" clipRule="evenodd" />
</svg> {item?.productId?.quantity || item?.quantity} units
            </div>
            </div>
      </div>
    <div
