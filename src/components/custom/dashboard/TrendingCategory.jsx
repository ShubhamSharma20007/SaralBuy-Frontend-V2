...
const TrendingCategory = ({ categories }) => {
  ...
  const handleNavigate = (categoryId) => {
    navigate(
      `/product-listing?title=%27%27&key=enter&category=${encodeURIComponent(categoryId)}`
  ...
  return (
    <div className="px-4  bg-no-repeat z-0 bg-cover  py-10  min-h-82  max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-5 px-5 sm:px-10  mt-10  ">
        {
          categories && categories.map((category, idx) => (<div
            key={idx}
            ...
            <p className="text-gray-600 text-center mt-3 capitalize">{category.category.categoryName}</p>
          </div>))
        }
      </div>
